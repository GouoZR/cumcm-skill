...
4. 核对符号、公式、假设、算法描述、结果数字、图号、表号和路径。
5. 对照模型规格、求解审计和结果文件，识别夸大、跳步、因果误述和未验证主张。
6. 审核文献元数据、证据等级与 claim map；国内来源优先但不以数量替代权威性。
7. 按需对照 `winning_patterns.md` 与 `anti_patterns.md`，但只把它们当经验性质量参考；官方规则相关内容必须基于已核验的当届通知。
