...
3. 每一问按 `artifacts/quality_contract.json` 建立“问题要求—分析单位与口径—方法—结果—解释—验证—适用边界”的闭环；不能只堆公式或代码过程。
4. 论文核心数字以 `results/result_registry.json` 为唯一索引，从登记的 `source` 与 `source_locator` 取值；不得凭聊天记录、旧草稿或人工记忆跨文件抄写。图表回链到 `figures/`，模型表述与 Stage 3 通过版本一致。
5. 正文写明关键指标的单位、方向、聚合范围和比较基准；局部敏感性、单次随机运行、相关分析、低保真近似和启发式最好值必须保留对应边界。
6. 只引用 `literature/library.json` 中已核验且由 claim map 支持的来源；metadata-only 记录不得支撑实质结论。
7. 若无可用文献服务，可使用用户提供或官方可核验来源；不得伪造引用。缺失证据写入阻断项或局限性。
8. 主 Agent 装配 `paper_draft.md` 前，先用 `competitions/cumcm/anti_patterns.md` 做自检，逐类清掉可机械发现的写作缺陷（摘要 A 类、假设与符号 B 类、结果分析 E 类、呈现 I 类），不把这些留给 Codex Stage 5 才发现。
9. 输出 `support_materials_manifest.md`，不生成 DOCX/PDF。
...
- 每一问闭环完整，关键数值来自结果注册表，并写清指标口径、单位、作用域、比较基准和现实解释；
