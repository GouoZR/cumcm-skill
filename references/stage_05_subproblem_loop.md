...
loads_reference: ["references/model_catalog.md", "competitions/cumcm/winning_patterns.md§5", "references/rubrics.md§Stage_5", "references/algorithms/README.md", "references/sciverse_guide.md§Stage_5"]
...
主 Agent (本 Agent — 协调 + 跨 Qi 一致性)
  ...
  └── 文献 Agent → Sciverse 文献检索
        ...
        工具: Sciverse MCP (semantic_search + search_papers + read_content)
```
...
for Qi in [Q1, Q2, ..., Qn]:
    ...
    B. 求解实现 (2-4h)
    C. 结果验证 (30 min)
    C.5 求解可信度证据链 (30-45 min, 随机/启发式/迭代求解器必做)
    D. 有证据需要时做子灵敏度
    E. 物理意义 (15 min)
...
要求:
- 每个变量、参数、约束都有编号
- 公式用 LaTeX 数学模式 `$...$` / `$$...$$` (在 Markdown 与主流编辑器中可读; stage 8 直接复制)
- 声称的松弛、复合或自适应机制必须出现在公式与代码中，并在结果中提供可核验证据；否则使用标准模型名

**多目标多资源分配建模（Q5 类）**：当 Qi 是"多资源 × 多目标 × 多投放件"的分配 + 调度问题，不要直接跳到"先手工分配再逐个优化"。按四步逻辑链建模：

1. 威胁/优先级评估模型（量化目标排序依据）；
2. 方案效能评估矩阵（资源×目标×件数的最优效能，稀疏正常并解释原因）；
3. 效用驱动贪心分配（按优先级查效能矩阵逐目标分配，分解为独立子问题）；
4. 每个子问题精细优化（PSO/SA/盆地跳跃等）。

分配阶段的每个决策必须来自一次可核验的效能评估，而不是拍脑袋分工。详见 `references/algorithms/README.md` 组合建模要点。
...
- 首行明确 "对应论文 §X" (`competitions/cumcm/winning_patterns.md` §10)
...
五步 sanity check (anti_pattern D2/D3):
...
5. **合理性 / 量级互校**: 将关键数值与**独立可查的领域基准**对照，防范"统计上成立但工程上荒谬"的解。典型例子:
   - **统计检验/抽样方案**: 方案的检验功效要有实际判别力。如二项抽样方案 (n, c) 须满足 `c < n`（c=n 时永不拒收 = 不检测，是伪解）；并校验两类错误概率都真实小于容限，而非"数学上可写但无判别力"。最小化 n 的朴素解常给出 n 个位数、判为"荒谬"，此时应扩大 n 到满足双风险约束或加置信度下限。
   - **运动学/动力学**: 速度、角速度量级必须与题面驱动输入同阶。若链条末端速度远偏离驱动速度（如相差两个数量级），通常是模型忽略了几何放大/曲率耦合，需回 A 检查约束构造，而非直接采信。
   - **优化结果**: 目标函数值的量级应与题面收益/成本参数同阶；若小数量级离谱，回查单位换算与目标口径。
...
### C.5 求解可信度证据链 (随机/启发式/迭代求解器必做, 30-45 min)

国赛评委对"结果可信"的判断优先于最终数值本身。**任何依赖随机采样、启发式或迭代优化的求解器，都必须产出可复核的可信度证据**，否则单次数值会被质疑"是不是偶然搜出来的"。

对每个采用随机/启发式/迭代求解的 Qi，必须产出四件证据（源数据写入 `results/`，论文 §5/§6 直接引用，不手填）：

1. **收敛曲线**: 画 迭代/采样次数 → 目标值 曲线，展示目标值随迭代上升并趋于稳定（国赛评委特别喜欢，直接提升结果可信度）。
2. **多次重复实验**: 同一组随机种子下重跑 N 次（如 10~30 次），报告 均值 / 标准差 / 最好 / 最坏；波动大时说明原因并补多种子评估。
3. **最优值分布**: 统计 N 次重复的最优值分布（直方图或箱线图），证明结果不是单次偶然。
4. **规模敏感性**: 对关键规模参数（如随机采样数、种群大小、迭代上限）做比较（如 10万/20万/50万/100万），展示最优值趋于稳定，证明已收敛到搜索能力的极限。

落地要求:
- 每个随机/启发式求解器固定并记录 `random.seed`（anti_pattern D4），多次重复实验用多种子集合。
- 收敛曲线、重复实验统计、规模敏感性图的源数据写入 `results/Qi_convergence.csv` 等文件，论文引用时从文件读取。
- 若 Qi 是确定性求解（解析解 / MILP 精确解 / 确定性仿真），无需以上四件，但需记录求解器状态与量级互校（Step C）。
- 若时间不足只做其中两件，优先 1 收敛曲线 + 2 多次重复实验。
...
**结果解释优先于结果展示**：为每个关键数值补一个"为什么是这个值而不是相邻值"的机制解释（如"最优角度 4.9° 而非 0°"，依据见 stage 6 的机理关系图）。这一问是国奖论文"论文深度"的最大杠杆，机制解释写入 `physical_meaning_summary`，stage 8 直接使用。
...
```json
{
  ...
  "credibility_evidence": {
    "convergence_curve": "results/Q1_convergence.csv",
    "repeated_runs": {"n_runs": 30, "mean": "...", "std": "...", "best": "...", "worst": "...", "seed_range": "..."},
    "optimum_distribution": "results/Q1_distribution.png",
    "scale_sensitivity": "results/Q1_scale_sensitivity.csv"
  },
