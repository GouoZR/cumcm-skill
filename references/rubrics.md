...
| 竞赛特化 dim 名 | baseline `scripts/score_artifact.py:DIM_WHITELIST`；cumcm overlay 未设 `dim_whitelist`（明确走 baseline fallback，见 `load_dim_whitelist`） | score_artifact.py 自动合并 |
