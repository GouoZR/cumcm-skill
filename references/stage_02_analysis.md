...
### Step 4.5: 输出模板规模推断 (10 min) — 若题目给结果模板

若题目附有结果模板（如 `result1_1.xlsx` 等），**扫描模板结构推断求解规模与算法选择**，写入 `data_schema`：

```
结果模板 (xlsx):
- sheet 数: <由扫描写入> (如 7 个 sheet = 规划期数)
- 行 × 列: <由扫描写入> (如 87 行 × 45 列)
- 隐含决策变量规模: <期数 × 地块数 × 季数 × 作物数> → 估算数值
- 求解难度推断: <easy | medium | hard + 理由> (对照 model_catalog 规模阈值)
- 算法提示: <精确求解器可行 / 需降维 / 需启发式>
```

> **价值**: 模板的维度直接反映决策变量的规模和结构。如 2024 C 题结果模板为 7 sheet × 87 行 × 45 列，可提前判断变量规模约 4.7 万，从而在 stage 3 就决定用精确 MILP 还是启发式，避免 stage 5 求解时才发现规模过大。
