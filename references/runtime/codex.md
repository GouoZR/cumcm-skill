...
6. Stage 1、3、5 开始后按需加载 `references/runtime/codex_subagents.md`；不要在其他阶段预加载。

## 工作边界

- Codex负责模型规格、关键推理、结果审计、论文终审和最终裁决。
- 原则上输出规格或结构化修改单，不大面积接管 Claude 的实现或论文正文。
- SubAgent 只做当前 Codex 阶段内部的独立侧向审查；对共享工作区仍由 `codex` 单一 owner 负责。
- 主 Agent 必须亲自完成关键路径、复核 SubAgent 证据、解决冲突、写正式产物并决定通过或回退。
- SubAgent 默认只读，不修改状态、共享产物、manifest 或 handoff；不可用时由主 Agent 串行执行同等独立复核。
- 交接前生成并校验 handoff；状态更新必须带 revision，冲突后重新读取，不覆盖。

## 质量目标

以可复现、可检验、逐问闭环和强证据链为默认标准。所谓“国奖级”是质量目标而非获奖承诺；历史高质量模式不得冒充官方阈值。Stage 3/5 存在已确认 blocker 或未闭环 high 问题时必须退回。
