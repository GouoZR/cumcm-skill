...
**机理分析图（决定性贡献）**：当优化结果给出一个"最优决策"（最优角度、速度、时刻等）时，必须补一张**决策变量—目标值关系图**，回答"为什么是最优值，而不是相邻值"。典型做法：

- 固定其他决策变量为最优值，扫描关键决策变量（如最优角度 4.9° 附近），画 角度 → 目标值 曲线或热力图，展示峰值位置与平坦区/尖锐峰形态；
- 若存在两个决策变量交互，画二维热力图（如 角度 × 速度 → 遮蔽时长），用 colorbar 标单位；
- 该图直接回应"为什么不是 0° / 为什么不是 10° / 为什么速度更快反而不好"这类评委必然追问的问题，是提升"论文深度"最高杠杆的一类图。

（本任务经 2026-08-02 A/B 题并行走查确认：stage 6 需同时产出 灵敏度 Tornado 图 + 关键决策变量的机理关系图；图的数量与类型记录在 `decision_log.stages.6.figures`，正文引用一致。）
...
**机理结论（与结果解释闭环）**：对每个关键决策变量，用关系图/热力图证据写出机理结论，形式为”最优值出现在 X，因为……；偏离到相邻值时目标如何变化”。例：”最优角度 4.9° 处存在尖锐峰值，偏离 ±5° 目标下降约 20%，说明云团部署对航向高度敏感”。这类结论让”为什么是这个值”落到证据上，避免只报告结果数值。
...
```json
{
  ...
  "mechanism_insights": "对每个关键决策变量: 最优值位置、相邻值变化趋势、为何如此(依据关系图/热力图)",
