...
<若为随机/启发式/迭代求解: 收敛证据、N 次重复实验统计、规模敏感性比较、O(·) 复杂度 — 数据从 results/ 读取，不手填>

![Q1 求解流程](figures/process_q1_flow.png)
*图 X: 问题一求解流程*

![Q1 收敛曲线](figures/process_q1_convergence.png)
*图 X: 问题一算法收敛曲线 (随机/启发式求解必配)*

### 5.1.3 结果、图表与现实解释
<数值结果 + 物理意义讨论>
<结果解释: 每个关键数值回答"为什么是这个值而非相邻值", 用 §6 的机理关系图支撑>
...
![决策机理](figures/sensitivity_mechanism.png)
*图 X: 关键决策变量—目标值关系图 (回答"为什么是这个最优值")*
...
> 条目按原始出版语言书写：原始文献为中文的（中文期刊、中文教材、官方文件等）题名保留中文原名，不得翻译成英文版；英文文献保留英文原名。
